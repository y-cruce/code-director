#!/usr/bin/env bash
# Copies the skill and the codex-task agent into ~/.claude/ (and removes the codex-worker agent left by older versions), then prints the snippet that must be added to CLAUDE.md by hand.
set -euo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"
DEST="${CLAUDE_HOME:-$HOME/.claude}"

mkdir -p "$DEST/skills/codex-director/scripts" "$DEST/agents"
rm -f "$DEST/agents/codex-worker.md"
cp "$HERE/agents/codex-task.md" "$DEST/agents/codex-task.md"
...
echo "  $DEST/agents/codex-task.md"
echo
