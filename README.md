...
**Prose sent to the agent does not reach Codex.** It returns `UNROUTED_MESSAGE:` without following; resend the correction using `MESSAGE_FILE:`. A bare `continue` only resumes following, and a new round requires the `DISPATCH_FILE:` / `CWD:` / `NAME:` triple. If the last report was `QUESTION`, a `MESSAGE_FILE:` request must explicitly state that it has already been answered through `answer`; otherwise the agent returns `MESSAGE_REFUSED job=<id> question pending, answer it first`. On refusal, answer first; on failure, fix the error or update the plugin before resending.
